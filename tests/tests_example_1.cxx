...
TEST_CASE("Microwave example", "[.][example][microwave]")
{
    ...
    // power on device
    sm.handle(PowerButtonPressed{});

    // open the door and put some food inside the microwave
    sm.handle(DoorOpen{});
    // close the doors and set the timer for 30s
    sm.handle(DoorClose{});
    sm.handle(SetTimer{std::chrono::seconds{30}});

    // press start button
    sm.handle(StartStopButtonPressed{});

    // microwaving for 15 seconds now...
    sm.handle(TimeElapsed{std::chrono::seconds{15}});

    // open to check the food state
    sm.handle(DoorOpen{});

    // nah, still not good, continue...
    sm.handle(DoorClose{});
    sm.handle(TimeElapsed{std::chrono::seconds{15}});

    // should be good now! take out food and power off microwave!
    sm.handle(DoorOpen{});
    sm.handle(PowerButtonPressed{});
}

// run as ./tests [microwave], should produce:
// PoweredOff state enter
// PoweredOff: PowerButtonPressed event
// PoweredOff state exit
// Ready state enter
// PoweredOn state enter
// Ready: DoorOpen event
// Ready state exit
// Opened state enter
// Opened: DoorClose event
// Opened state exit
// Ready state enter
// Ready: SetTimer event
// Ready: StartStopButtonPressed event
// Ready state exit
// Microwaving state enter
// Microwaving: TimeElapsed event
// Microwaving: DoorOpen event
// Microwaving state exit
// Paused state enter
// Paused: DoorClose event
// Paused state exit
// Microwaving state enter
// Microwaving: TimeElapsed event
// Microwaving state exit
...
// Ready state exit
// Opened state enter
// PoweredOn: PowerButtonPressed event
// PoweredOn state exit
// Opened state exit
// PoweredOff state enter
// PoweredOff state exit